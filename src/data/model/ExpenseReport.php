...
class ExpenseReport {
	...
	public function validate() {
		if ( strlen( $this->name ) > 100 ) {
			throw new ValidationException( 'name', 'Namn får inte vara längre än 100 tecken.' );
		}
		if ( strlen( $this->email ) > 100 ) {
			throw new ValidationException( 'email', 'Epost får inte vara längre än 100 tecken.' );
		}
		if ( ! Person::is_valid_email_address( $this->email ) ) {
			throw new ValidationException( 'email', 'Epost verkar inte vara en giltig adress.' );
		}
		...
		if ( strlen( $this->bank_account ) > 100 ) {
			throw new ValidationException( 'bank_account', 'Bankkontot får inte vara längre än 100 tecken.' );
		}
	}
